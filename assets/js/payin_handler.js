...
const RIGID_ACCOUNT_RULES = {
    ...
    "ZMW": { regex: /^(260|0)?\d{9}$/, error: "赞比亚通道校验失败：必须是标准的赞比亚钱包账号/手机号！" }, // 🔒 补焊：对齐 ZMW 现代防爆正则
    "MWK": { regex: /^(265|0)?(8|9)\d{8}$/, error: "马拉维通道校验失败：必须是合规的 M-Pesa/Airtel 钱包号！" }, // 🔒 补焊：对齐 MWK
    "RWF": { regex: /^(250|0)?7\d{8}$/, error: "卢旺达通道校验失败：必须是标准的卢旺达钱包手机号！" }, // 🔒 补焊：对齐 RWF
