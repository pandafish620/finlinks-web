...
// 版本说明：v2.7.0-FX-Closure-Locking (参数作用域锁死完全体，彻底斩断时钟丢失引发的 408 熔断)
...
const BACKEND_ENV = {
    ...
    endpoints: {
        ...
        payin_callback: "/webhooks/payin/callback"  // 🌟 核心网关血管：离岸代收异步核销
    }
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!token || token === "MOCK_DEVELOPER_TOKEN" || token === "admin_sandbox_pass" || !token.includes(".")) {
        ...
        // 确保满足安全舱解析时最起码的结构测距，彻底击碎 "Not enough segments" 排异
        const mockHeader = btoa(JSON.stringify({ alg: "HS256", typ: "JWT" }));
        const mockPayload = btoa(JSON.stringify({ sub: "admin", exp: 2095215455 })); // 100% 对齐后端打印出的 {'sub': 'admin'}
...
async function fetchBalances() {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            // 🛡️ 核心风控：只有当商户停留在持仓大厅面板时，才频繁洗刷卡片 DOM，大幅节约浏览器渲染资源
            ...
            // 联动多通道感知：如果后端随单吐回了活体多通道遥测轨迹，秒级将其推入底部的黑色日志框中
        ...
        if (response.status === 200) {
            ...
            if (fxWidgetInfo) {
                fxWidgetInfo.innerHTML = `
                    ...
                    <span class="block mt-1 text-[10px] text-slate-500">点差锁定通道: ${quote.routing_via} | 3秒限时 TTL 锁价保护</span>
                `;
            }

            // 🎨 3. 呼唤集成了 3s TTL 倒计时条的强确权弹窗舱
            showPremiumNotification(
                ...
                true, 
                () => {
                    // 💡 【战役 B 核心自愈】：拒绝依赖不稳定的全局变量，直接将当前作用域内纯净、实时的活体参数注入交割执行链
                    submitFxConversion(quote.final_settlement_rate, quote.quote_timestamp, quote.routing_via);
                },
                () => {
                    pushAuditLog(`[FX CANCELLED] 操盘手取消或 3秒未授权！即期报价作废，零敞口风险。`);
                },
                3 
            );
...
// 💱 提交即期外汇双边原子化换汇（支持闭包参数强锁死注入，抗心跳抖动）
async function submitFxConversion(forcedRate = null, forcedTimestamp = null, forcedRouting = null) {
    ...
    // 🛡️ 优先读取强绑定的闭包参数，完全规避全局时钟被轮询刷新冲掉而退化为 0 的系统隐患
    ...
    try {
        // 🚀 URL 参数完美组装，彻底斩断 quote_timestamp=0 导致的 17 亿秒延迟时间硬穿仓
        ...
        // 💡 异步解包自愈：捕获非 200 状态码下 FastAPI 吐出的原始结构体
        ...
        if (response.status === 200) {
            ...
            // 🛡️ 绝杀 [object Object]：精准提取 detail 字符串，让上游拒绝底牌见光
...
async function triggerMockPayinCallback() {
    pushAuditLog("[PAYIN DISPATCH] 商户触发 Demo Day 活体收单流量模拟注入，异步报文开始外发...");
    
    const randomGatewayRef = "TX-PAYIN-DEMO-" + Math.floor(Math.random() * 10000000);
    
    try {
        const url = `${BACKEND_ENV.BASE_URL}${BACKEND_ENV.endpoints.payin_callback}?merchant_id=admin&amount=5000&currency=NGN&gateway_reference=${randomGatewayRef}`;
        
        const response = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" } });
        const result = await response.json();

        if (response.status === 200 && result.status === "acknowledged") {
            pushAuditLog(`[WEBHOOK ACK] 离岸代收清算成功！上游渠道参考号: ${randomGatewayRef}`);
            
            const currentWaterline = result.current_balance !== undefined ? result.current_balance : "已入账等待轮询";
            pushAuditLog(`[LEDGER UPDATED] 影子总账原子化记账完成。系统内部流水水线: ${currentWaterline}`);
            
            showPremiumNotification(
                "📥 FinLinks 离岸收单对撞成功",
                `商户主体：<span class="text-slate-100 font-mono">${result.merchant}</span><br>` +
                `账实到账：<span class="text-emerald-400 font-mono font-bold">+${result.credited_amount} ${result.credited_currency}</span><br>` +
                `当前总账可用水线：<span class="text-indigo-400 font-mono font-bold">${typeof currentWaterline === 'number' ? currentWaterline.toLocaleString(undefined, {minimumFractionDigits: 2}) : currentWaterline} NGN</span>`,
                "emerald",
                false, 
                () => { fetchBalances(); }
            );
        } else {
            pushAuditLog(`[WEBHOOK DENIED] 外部清算报文被中台风控拦截: ${result.detail || "未知异常"}`);
            showPremiumNotification("⚠️ 收单清算熔断", result.detail || "未知单边账风险", "rose", true);
        }
    } catch (error) {
        pushAuditLog("[WEBHOOK TIMEOUT] 收单回调网关发生物理网络通信超时。");
        showPremiumNotification("❌ 通信突发中断", "网络异常，模拟电报未能击穿网关", "rose", true);
    }
}

// 🛠️ 辅助 UI 组件：免阻塞机构级黑金浮层通知窗（金融双向渠道确权版）
    ...
    if (countdownTtl > 0) {
        ...
        finlinksLiveTtlTimer = setInterval(() => {
            ...
            if (currentStep <= 0) {
                ...
                pushAuditLog(`[TTL EXPIRED] ⏳ 3秒内未授权！RFS 固价合同作废，指令强制回滚流产。`);
...
// =================================================================
// 💸 旗舰级总线：FXAll 3秒独占固价背对背放款核销大闸
// =================================================================
window.executeLivePayoutDisbursal = async function() {
    ...
    pushAuditLog(`[RFS ACTIVATE] 正在向中台流动性池索要 3秒独占锁价合同...`);
    ...
    try {
        ...
        if (response.status === 200 && result.status === "preview") {
            pushAuditLog(`[QUOTE CAPTURED] 报价单锁死成功！编号: ${result.quote_id}。请在 3秒内签署终审授权。`);
        ...
        showPremiumNotification("❌ 物理中台脱轨", "无法叩开后端网关，请核实后台 Uvicorn 指针", "rose", true);
    }
};